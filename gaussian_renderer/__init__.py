...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size: float, scaling_modifier = 1.0, override_color = None, subpixel_offset=None):
    ...
    if viewpoint_camera.FoVx > 6.0:
        tanfovx = 1e10
    else:
        tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)

    if viewpoint_camera.FoVy > 3.0:
        tanfovy = 1e10
    else:
    ...
    pano_theta_min = float(getattr(viewpoint_camera, "pano_theta_min", -0.5 * math.pi))
    pano_theta_max = float(getattr(viewpoint_camera, "pano_theta_max", 0.5 * math.pi))
