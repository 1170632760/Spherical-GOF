...
__device__ float4 computeCov2D(const float3& mean, float W, float H, float tan_fovx, float tan_fovy, float focal_x, float focal_y, float kernel_size, const float* cov3D, const float* viewmatrix)
{
	...
	glm::mat3 J = glm::mat3(
		focal_x / r, 0.0f, -(focal_x * t.x) / (r * r),
		0.0f, focal_y / r, -(focal_y * t.y) / (r * r),
		0, 0, 0);
...
__global__ void preprocessCUDA(int P, int D, int M,
	...
	const float pano_theta_min, const float pano_theta_max,
	...
	bool prefiltered)
{
	...
	radii[idx] = 0;
	tiles_touched[idx] = 0;
	conic_opacity[idx] = { 0.f, 0.f, 0.f, 0.f };
	radii_xy[idx] = make_int2(0, 0);
	points_xy_image[idx] = { 0.f, 0.f };
	if (view2gaussians != nullptr)
	{
		for (int ii = 0; ii < 10; ii++)
			view2gaussians[idx * 10 + ii] = 0.f;
	}
	...
	float R = sqrt(p_view.x*p_view.x + p_view.y*p_view.y + p_view.z*p_view.z);
	float inv_R = 1.0f / max(R, 1e-6f);
	const float theta_span = fmaxf(pano_theta_max - pano_theta_min, 1e-6f);
	float sin_theta = p_view.y * inv_R;
	sin_theta = min(1.0f, max(-1.0f, sin_theta));
	float phi = atan2(p_view.x, p_view.z);
	float theta = asin(sin_theta);

	float u = phi / (2.0f * 3.14159265358979323846f) + 0.5f;
	float v = (theta - pano_theta_min) / theta_span;
	u = fminf(1.0f, fmaxf(0.0f, u));

	float2 point_image = { u * (W - 1), v * (H - 1) };
	...
	float4 cov = computeCov2D(p_orig, W, H, tan_fovx, tan_fovy, focal_x, focal_y, kernel_size, cov3D, viewmatrix);
	...
	const float kAlphaCut = 1.0f / 255.0f;
	const float kBoundSigma = sqrtf(-2.0f * logf(kAlphaCut));
	float max_s = max(scale.x, max(scale.y, scale.z)) * scale_modifier;
	float radius_world = max_s * kBoundSigma;
	float alpha = asin(min(1.0f, radius_world / (R + 1e-6f)));
	float delta_theta = alpha;

	float pixel_half_theta = theta_span / (2.0f * max(1, H - 1));
	float pole_margin = 0.5f * PI / 180.0f;
	float lat_extent = alpha + pixel_half_theta + pole_margin;
	bool full_longitude = (fabsf(theta) + lat_extent >= 0.5f * PI);
	float delta_phi;
	if (full_longitude)
	{
	...
	{
		float cos_theta2 = max(1e-8f, 1.0f - sin_theta * sin_theta);
		float cos_theta = sqrtf(cos_theta2);
		float sin_delta_phi = sinf(alpha) / max(1e-8f, cos_theta);
		sin_delta_phi = min(1.0f, max(-1.0f, sin_delta_phi));
		delta_phi = asinf(sin_delta_phi);
	}

	float radius_px_x = (delta_phi / (2.0f * PI)) * (W - 1);
	float radius_px_y = (delta_theta / theta_span) * (H - 1);
	if (full_longitude)
	{
		radius_px_x = (float)(W - 1);
	}
	int radius_x = (int)ceil(radius_px_x);
	int radius_y = (int)ceil(radius_px_y);
	float my_radius = max((float)radius_x, (float)radius_y);

	uint2 rect_min, rect_max;
	getRectXYWrapX(point_image, radius_x, radius_y, rect_min, rect_max, grid, W);
	...
	const bool is_panorama = (tan_fovx > 1e8f) && (tan_fovy > 1e8f);
	depths[idx] = is_panorama ? R : p_view.z;
	radii[idx] = my_radius;
	points_xy_image[idx] = point_image;
	radii_xy[idx] = make_int2(radius_x, radius_y);
...
renderCUDA(
	...
	float pano_theta_min, float pano_theta_max,
	...
	float* __restrict__ out_color)
{
	...
	float3 ray_point = get_spherical_ray(pixf.x, pixf.y, W, H, pano_theta_min, pano_theta_max);
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float normal[3] = {
				view2gaussian_j[0] * ray_point.x + view2gaussian_j[1] * ray_point.y + view2gaussian_j[2] * ray_point.z,
				view2gaussian_j[1] * ray_point.x + view2gaussian_j[3] * ray_point.y + view2gaussian_j[4] * ray_point.z,
				view2gaussian_j[2] * ray_point.x + view2gaussian_j[4] * ray_point.y + view2gaussian_j[5] * ray_point.z
			};

			// use AA, BB, CC so that the name is unique
			double AA = ray_point.x * normal[0] + ray_point.y * normal[1] + ray_point.z * normal[2];
			double BB = 2 * (view2gaussian_j[6] * ray_point.x + view2gaussian_j[7] * ray_point.y + view2gaussian_j[8] * ray_point.z);
	...
	bool prefiltered)
{
	...
	if (v < 0.0f || v > 1.0f)
		return;
	const float v_eps = 0.5f / max(1, H - 1);
	v = fminf(1.0f - v_eps, fmaxf(v_eps, v));
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int k = 0; k < 5; ++k){
				...
				float3 ray_point = get_spherical_ray(
					pixf.x + offset_xs[k],
					pixf.y + offset_ys[k],
					W,
					H,
				...
				const float normal[3] = { view2gaussian_j[0] * ray_point.x + view2gaussian_j[1] * ray_point.y + view2gaussian_j[2] * ray_point.z,
									      view2gaussian_j[1] * ray_point.x + view2gaussian_j[3] * ray_point.y + view2gaussian_j[4] * ray_point.z,
									      view2gaussian_j[2] * ray_point.x + view2gaussian_j[4] * ray_point.y + view2gaussian_j[5] * ray_point.z};

				// use AA, BB, CC so that the name is unique
				float AA = ray_point.x * normal[0] + ray_point.y * normal[1] + ray_point.z * normal[2];
				float BB = 2 * (view2gaussian_j[6] * ray_point.x + view2gaussian_j[7] * ray_point.y + view2gaussian_j[8] * ray_point.z);
				float CC = view2gaussian_j[9];
