...
#define MAX_NUM_CONTRIBUTORS 1024
...
__forceinline__ __device__ void getRectXYWrapX(const float2 p, int radius_x, int radius_y, uint2& rect_min, uint2& rect_max, dim3 grid, int image_width)
{
	rect_min = {
	...
	if (p.x - radius_x < 0.0f || p.x + radius_x >= image_width)
	{
		rect_min.x = 0;
		rect_max.x = grid.x;
	}
...
__forceinline__ __device__ float3 get_spherical_ray(
	float x,
	float y,
	int W,
	int H,
	float pano_theta_min,
	float pano_theta_max)
{
    float u = x / max(1, W - 1);
    float v = y / max(1, H - 1);
    ...
    float phi = (u - 0.5f) * 2.0f * 3.14159265358979323846f;
    const float theta_span = fmaxf(pano_theta_max - pano_theta_min, 1e-6f);
    float theta = pano_theta_min + v * theta_span;
    ...
    return { sin_phi * cos_theta, sin_theta, cos_phi * cos_theta };
}
	...
	float3& p_view)
{
	...
	if (p_view.x*p_view.x + p_view.y*p_view.y + p_view.z*p_view.z <= 0.2f * 0.2f)
	{
		if (prefiltered)
		{
