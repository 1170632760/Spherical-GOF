	...
	float* __restrict__ dL_dview2gaussian)
{
	...
	float3 ray = get_spherical_ray((float)pix.x, (float)pix.y, W, H, pano_theta_min, pano_theta_max);
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicAdd(&(dL_dview2gaussian[global_id * 10 + 2]), dL_dnormal[0] * ray.z + dL_dnormal[2] * ray.x);
			atomicAdd(&(dL_dview2gaussian[global_id * 10 + 3]), dL_dnormal[1] * ray.y);
			atomicAdd(&(dL_dview2gaussian[global_id * 10 + 4]), dL_dnormal[1] * ray.z + dL_dnormal[2] * ray.y);
			atomicAdd(&(dL_dview2gaussian[global_id * 10 + 5]), dL_dnormal[2] * ray.z);
			atomicAdd(&(dL_dview2gaussian[global_id * 10 + 6]), dL_dB * 2 * ray.x);
			atomicAdd(&(dL_dview2gaussian[global_id * 10 + 7]), dL_dB * 2 * ray.y);
			atomicAdd(&(dL_dview2gaussian[global_id * 10 + 8]), dL_dB * 2 * ray.z);
