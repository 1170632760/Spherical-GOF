	...
	int image_width)
{
	...
	if (radii_xy[idx].x > 0 && radii_xy[idx].y > 0)
	{
		// Find this Gaussian's offset in buffer for writing keys/values.
		uint32_t off = (idx == 0) ? 0 : offsets[idx - 1];
		uint2 rect_min, rect_max;

		getRectXYWrapX(points_xy[idx], radii_xy[idx].x, radii_xy[idx].y, rect_min, rect_max, grid, image_width);
...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	obtain(chunk, geom.internal_radii_xy, P, 128);
	...
	bool debug)
{
	...
	const float focal_y = height / theta_span;
	const float focal_x = width / (2.0f * 3.14159265358979323846f);
