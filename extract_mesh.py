...
def marching_tetrahedra_with_binary_search(
    ...
    if max_tetra_points > 0 and n_full > max_tetra_points:
        keep = torch.randperm(n_full, device=points.device)[:max_tetra_points]
        points = points[keep]
        if points_scale is not None and points_scale.shape[0] == n_full:
            points_scale = points_scale[keep]

    cells_path = os.path.join(render_path, "cells.pt")
    # load cell if exists
    if (not rebuild_cells) and os.path.exists(cells_path):
        print("load existing cells")
        cells = torch.load(cells_path)
        if cells.numel() > 0 and int(cells.max().item()) >= points.shape[0]:
    ...
    eval_views = views[::max(1, view_stride)]
    if len(eval_views) == 0:
        eval_views = views

    # evaluate alpha
    alpha = evaluage_alpha(points, eval_views, gaussians, pipeline, background, kernel_size)
    ...
    n_binary_steps = max(2, int(binary_steps))
    for step in range(n_binary_steps):
        print("binary search in step {}".format(step))
        mid_points = (left_points + right_points) / 2
        alpha = evaluage_alpha(mid_points, eval_views, gaussians, pipeline, background, kernel_size)
        ...
        if step != (n_binary_steps - 1):
            continue

        if texture_mesh:
            _, color = evaluage_alpha(points, eval_views, gaussians, pipeline, background, kernel_size, return_color=True)
...
def extract_mesh(
    ...
    with torch.no_grad():
        ...
        marching_tetrahedra_with_binary_search(
            ...
            near,
            far,
            max_tetra_points=max_tetra_points,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_tetra_points", default=300000, type=int)
    parser.add_argument("--view_stride", default=2, type=int)
    parser.add_argument("--binary_steps", default=5, type=int)
    parser.add_argument("--rebuild_cells", action="store_true", help="ignore cached fusion/cells.pt and rebuild tetra connectivity")
    ...
    extract_mesh(
        ...
        args.near,
        args.far,
        max_tetra_points=args.max_tetra_points,
        view_stride=args.view_stride,
        binary_steps=args.binary_steps,
        rebuild_cells=args.rebuild_cells,
    )
